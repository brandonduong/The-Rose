...
public class LoseThornsPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public static final String POWER_ID = ModInitializer.makeID("LoseThornsPower");
    ...
    public void atEndOfTurn(boolean isPlayer) { // At the start of your turn
        ...
        if (owner.hasPower("Thorns") && owner.getPower("Thorns").amount == 1) {
            this.addToBot(new RemoveSpecificPowerAction(owner, owner, "Thorns"));
        }

        // Lose Thorns
        else if (owner.hasPower("Thorns")) {
            this.addToBot(new ApplyPowerAction(this.owner, this.owner, new ThornsPower(this.owner, -this.amount), -this.amount));

        }

        // Remove LoseThornsPower
        this.addToBot(new RemoveSpecificPowerAction(this.owner, this.owner, "theRose:LoseThornsPower"));
    }
